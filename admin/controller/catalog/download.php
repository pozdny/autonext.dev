...
class ControllerCatalogDownload extends Controller {
	...
	public function delete() {
        ...
        if(isset($this->request->get['links_id'])){
            $url = '';
            $this->model_catalog_links_page->deleteLinks($this->request->get['links_id']);
            $this->response->redirect($this->url->link('catalog/download', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        }
	...
	protected function getList() {
       ...
       if(isset($this->request->post['submit_download'])){
            $temp = $this->request->files['name_file']['tmp_name'];
            $name_file = $this->request->files['name_file']['name'];
            move_uploaded_file($temp, "../down-files/" . $name_file);
           $data['update_link'] = $this->url->link('catalog/download', 'token=' . $this->session->data['token'] . $url, 'SSL');
        }
        ...
        $categories = $this->model_catalog_download->getCategoriesTop(0);
        foreach ($categories as $category) {
            if ($category['top']) {
                $url_id = '';
                $new_arr = array();
                $arr_links = array();
                $res = $this->model_catalog_links_page->getLinks($category['category_id']);
                if($res > 0){
                    foreach($res as $key=>$arr){
                        foreach($arr as $key2=>$value){
                            $new_arr[$key2] = $value;
                            if($key2 == 'link_id'){
                                $url_id = '&links_id='.$value;
                            }

                        }
                        $new_arr['delete_links'] = $this->url->link('catalog/download/delete', 'token=' . $this->session->data['token'] . $url_id, 'SSL');
                        $arr_links[$key] = $new_arr;
                        $new_arr = null;
                    }
                }
        ...
        if (isset($this->request->post['submit_download'])) {
            $data['name_file'] = $this->request->files['name_file'];
            $data['opt_page'] = $this->request->post['opt_page'];
            $data['name_link'] = $this->request->post['name_link'];
            $data['data'] = array(
                'name' => $data['name_file']['name'],
                'opt_page' => $data['opt_page'],
                'name_link' => $data['name_link']
            );
            $this->model_catalog_download->addFiles($data['data']);
    ...
    public function update() {
        ...
        $res = $this->model_catalog_links_page->updateLink($data);
        if($res){
            $result = 'ok';
        }
        else{
