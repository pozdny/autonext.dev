	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['logo'] = $this->config->get('config_url') . 'image/catalog/logo_mail.png';
                ...
                $data['fio'] = $order_info['payment_firstname'];
                $data['config_mail_alert'] = $this->config->get('config_mail_alert');
                $data['bottom_info'] = 'Если у Вас есть какие-либо вопросы позвоните по т.8(383) 292-58-19';
				...
				foreach ($order_product_query->rows as $product) {
					...
					$data['products'][] = array(
						...
						'price'    => $this->currency->format($this->currency->convert($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $this->config->get('config_currency'), $this->config->get('config_currency_out')), $this->config->get('config_currency_out'), ''),
                        'total'    => $this->currency->format($this->currency->convert($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $this->config->get('config_currency'), $this->config->get('config_currency_out')),  $this->config->get('config_currency_out'), '')
);
				}
				...
				foreach ($order_voucher_query->rows as $voucher) {
					$data['vouchers'][] = array(
						...
						'amount'      => $this->currency->format($this->currency->convert($voucher['amount'], $this->config->get('config_currency'), $this->config->get('config_currency_out')), $this->config->get('config_currency_out'), ''),
					);
				}
				...
				foreach ($order_total_query->rows as $total) {
					$data['totals'][] = array(
						'title' => $total['title'],
						'text'  => $this->currency->format($this->currency->convert($total['value'], $this->config->get('config_currency'), $this->config->get('config_currency_out')),  $this->config->get('config_currency_out'), '')
				...
				if (!$this->model_payment_amazon_checkout->isAmazonOrder($order_info['order_id'])) {
					...
					foreach ($order_voucher_query->rows as $voucher) {
						$text .= '1x ' . $voucher['description'] . ' ' . $this->currency->format($this->currency->convert($voucher['amount'], $this->config->get('config_currency'), $this->config->get('config_currency_out')), $this->config->get('config_currency_out'), '');
					}
					...
					foreach ($order_total_query->rows as $total) {
						$text .= $total['title'] . ': ' . html_entity_decode($this->currency->format($this->currency->convert($total['value'], $this->config->get('config_currency'), $this->config->get('config_currency_out')), $this->config->get('config_currency_out'), ''), ENT_NOQUOTES, 'UTF-8') . "\n";
					}
