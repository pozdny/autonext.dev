...
<h3 style="margin-top:0;">Для заказа оборудования для Вашей станции, пожалуйста, заполните форму ниже</h3>
<div class="col-xs-12" style="margin-bottom: 50px; padding:20px 40px; background-color: #dde1da; border: 1px solid #a38d81">
<form action="contact_form.php" role="form" method="post" id="formSend">
    <div class="form-group col-xs-12">
        <label><span style="color:red;">*</span>&nbsp;Перечень оборудования</label>
    ...
    <div class="form-group col-xs-12">
        <button type="submit" class="btn btn-primary" name="order_equipment" >Отправить</button>
    </div>
</form>
</div>
    ...
    function validateint(form){
        ...
        inputs.each(function(){
            ...
            if(!valid && val_int.length > 0){
                $(this).addClass('er-in') ;
                $(this).next('span').html("Номер должен быть от 5 до 15 символов!");
                f = false;
            }
